...
```shell
    1. 每台机器都有自己的本地时间,所有要保证多个机器之前的时间一致,常采用 NTP(Network Time Protocol),根据一组专门的时间服务器来
       调整本地时间, 时间服务器则从精确更高的时间源(如 GPS 接收机)获取高精度时间.
    2. 单调时钟与墙上时钟
    ...
    5. 全局快照的同步时钟
            (1) 快照隔离的实现需要全局递增的事务 ID, 在分布式系统中,多节点采用同一套事务 ID, 可以采用时钟当事务 ID,
                google Spanner 做法是获取时钟的置信区间,如果有两个置信区间，两个置信区间都包含最早和最新可能的时间戳
                A = [A(earlylist), A(lastest)], B = [B(earlylist), B(lastest)], 如果 A(earlylist) < A(lastest)
                < B(earlylist) < B(lastest), 则认为 B 要晚于 A.
    6. 进程暂停
            (1) 分布式系统中的一个节点必须假定，执行过程中的任何时刻都可能被暂停相当长一段时间，包括运行在某个函数中间。
                暂停期间，整个集群的其他部分都在照常运行，甚至会一致将暂停的节点宣告为故障节点。
                暂停的节点可能会回来继续运行，除非再次检查时钟，否则它对刚刚过去的暂停毫无意识。
```
